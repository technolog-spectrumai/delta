...
@login_required
def quiz_practice(request, pk):
    """Practice pool: serve one unsolved task at a time (spec F-04/F-06).

    A correct answer removes the task from the pool; a wrong answer leaves it in
    and reveals the worked solution ("spróbuj ponownie za chwilę"). Progress is
    solved / total for this quiz; the section is complete when all are solved.
    """
    quiz = get_object_or_404(Quiz.objects.prefetch_related("questions"), pk=pk)
    if not (quiz.is_published or request.user.is_staff):
        raise Http404

    participant = _get_participant(request)
    if not participant:
        messages.error(request, "You need a community profile to practise.")
        return redirect("quizzes:quiz-detail", pk=quiz.pk)
    ...
    if request.method == "POST":
        ...
        if question.question_type == QuizQuestion.TYPE_OPEN:
            is_correct = question.check_open(request.POST.get("open_answer", ""))
        elif question.is_multiple_choice:
            is_correct = question.check_choice(request.POST.getlist("answer"))
        else:
            raw = request.POST.get("answer")
            is_correct = question.check_choice([raw] if raw else [])
        ...
        context = {
            ...
            "after_id": question.pk,
            "stats": quiz.practice_stats(participant),
        }
        return render(request, "quizzes/practice.html", PageProcessor().decorate(context, request))

    # GET — show the next unsolved task (optionally skipping the one just answered).
    ...
    question = quiz.next_unsolved_question(participant, after_id=after_id)
    context = {
        "quiz": quiz,
        "question": question,
        "answers": list(question.answers.all()) if question else [],
        "feedback": None,
        "after_id": None,
        "stats": quiz.practice_stats(participant),
    }
