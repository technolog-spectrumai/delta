...
# The real media/ dir is a docker-owned bind mount; write test files to a temp dir.
_VOD_MEDIA = tempfile.mkdtemp(prefix="delta-vod-test-")
...
@override_settings(MEDIA_ROOT=_VOD_MEDIA)
class VaultFilePlayViewTests(TestCase):
    @classmethod
    def tearDownClass(cls):
        super().tearDownClass()
        shutil.rmtree(_VOD_MEDIA, ignore_errors=True)
...
# ---------------------------------------------------------------------------
# Play-plugin registration (via vault autodiscover)
# ---------------------------------------------------------------------------

class VaultPlayPluginRegistrationTests(TestCase):
    def test_video_and_audio_plugins_registered(self):
        from toto.vault.plugins import VaultPlayPlugin

        video = VaultPlayPlugin.for_file_type("video")
        audio = VaultPlayPlugin.for_file_type("audio")
        self.assertIsNotNone(video)
        self.assertIsNotNone(audio)

    def test_video_plugin_points_at_play_view(self):
        from types import SimpleNamespace

        from toto.vault.plugins import VaultPlayPlugin

        video = VaultPlayPlugin.for_file_type("video")
        url = video.get_play_url(SimpleNamespace(pk=7))
        self.assertEqual(url, reverse("vod:vault_file_play", args=[7]))
